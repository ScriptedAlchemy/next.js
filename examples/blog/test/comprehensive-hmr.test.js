...
test("Comprehensive HMR Test - All Methods", async (t) => {
  ...
  console.log("========================================");
  console.log(
    "Testing all HMR approaches with proper timing and initialization",
  );
  console.log("");
  ...
  devServer.stdout.on("data", (data) => {
    ...
    if (
      output.includes("Hot Reloader Patch") ||
      output.includes("Efficient HMR")
    ) {
  ...
  try {
    ...
    assert.strictEqual(
      initialVisit.status,
      200,
      "Initial page should return 200",
    );
    assert.ok(
    ...
    try {
      ...
      console.log(
        `   📖 Read compiled _document.js: ${originalContent.length} bytes`,
      );
      ...
      if (updatedContent === originalContent) {
        ...
        if (updatedPage.body.includes("Hello world!!")) {
          ...
          console.log(
            "⚠️  File-Based HMR: Triggered but page not visibly updated",
          );
        }
  ...
  try {
    const internalAPITest = await makeRequest("/api/internal-api-hmr", "POST", {
      action: "test",
    });
    ...
    if (internalAPITest.status === 200) {
      ...
      console.log(
        "   Initializing (this may take time to create new compiler)...",
      );
      const initResponse = await makeRequest(
        "/api/internal-api-hmr",
        "POST",
        { action: "initialize" },
        30000,
      );
      ...
      if (initResponse.status === 200 && initResponse.body.success) {
        ...
        // Test HMR trigger
        const hmrTrigger = await makeRequest(
          "/api/internal-api-hmr",
          "POST",
          {
            action: "trigger-hmr",
        ...
        if (hmrTrigger.status === 200 && hmrTrigger.body.success) {
          ...
          if (updatedPage.body.includes("Hello world!!")) {
            ...
            console.log(
              "⚠️  Internal API HMR: Triggered but page not visibly updated",
            );
          }
  ...
  try {
    const efficientAPITest = await makeRequest("/api/efficient-hmr", "POST", {
      action: "test",
    });
    ...
    if (efficientAPITest.status === 200) {
      ...
      // Test initialization
      console.log(
        "   Initializing (attempting to capture existing hot reloader)...",
      );
      const initResponse = await makeRequest(
        "/api/efficient-hmr",
        "POST",
      ...
      if (initResponse.status === 200 && initResponse.body.success) {
        ...
        if (hmrTrigger.status === 200 && hmrTrigger.body.success) {
          ...
          if (updatedPage.body.includes("Hello world!!")) {
            ...
            console.log(
              "⚠️  Efficient HMR: Triggered but page not visibly updated",
            );
          }
  ...
  try {
    ...
    assert.strictEqual(
      markdownPageInitial.status,
      200,
      "Markdown page should return 200",
    );
    assert.ok(
      markdownPageInitial.body.includes("PAGE_HMR_AREA"),
    ...
    try {
      ...
      const originalMarkdownContent = await fs.readFile(
        markdownServerPath,
        "utf8",
      );
      console.log(
        `   📖 Read compiled markdown.js: ${originalMarkdownContent.length} bytes`,
      );
      ...
      if (updatedMarkdownContent === originalMarkdownContent) {
        ...
        try {
          ...
          const clearMarkdownCache = await makeRequest(
            "/api/server-hmr",
            "POST",
            {
              action: "clear-module-cache",
          ...
          console.log(
            "   ✅ Enhanced markdown page cache cleared (Nextra-aware)",
          );
        } catch (error) {
          console.log("   ⚠️  Cache clear failed, continuing anyway");
        }
        ...
        console.log(
          `   First markdown reload status: ${firstMarkdownReload.status}`,
        );
        ...
        console.log(
          `   Second markdown reload status: ${secondMarkdownReload.status}`,
        );
        ...
        if (
          updatedMarkdownPage.body.includes("HMR SUCCESS ON MARKDOWN PAGE!")
        ) {
          ...
          console.log(
            "⚠️  Markdown Page HMR: Triggered but page not visibly updated",
          );
          results.markdownHMR = {
            name: "Markdown Page HMR",
            available: true,
  ...
  const availableMethods = Object.values(results).filter(
    (r) => r.available,
  ).length;

  console.log(
    `🎯 Final Results: ${workingMethods} working / ${availableMethods} available / 3 total`,
  );
