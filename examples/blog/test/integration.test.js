...
test('HMR Integration Test', async (t) => {
  ...
  for (const endpoint of endpoints) {
    try {
      const response = await makeRequest(endpoint.path, 'POST', { action: 'test' });
  ...
  if (endpointResults['File-based HMR']?.available) {
    ...
    try {
      ...
      const hasHelloWorldInitially = initialPage.body.includes('<h1>Hello world!!</h1>');
      console.log(`   Initial page state: ${hasHelloWorldInitially ? 'HAS' : 'NO'} Hello World`);
      ...
      if (hmrResponse.status === 200 && hmrResponse.body.success) {
        ...
        const hasHelloWorldAfter = updatedPage.body.includes('<h1>Hello world!!</h1>');
        ...
        if (hasHelloWorldAfter && !hasHelloWorldInitially) {
          ...
          console.log('   ✅ Page contains expected content (may have been updated)');
        } else {
          console.log('   ⚠️  Page not visibly updated but HMR succeeded');
        }
      } else {
        console.log(`   ❌ File-based HMR failed: ${hmrResponse.body?.error || 'Unknown error'}`);
      }
    } catch (error) {
    ...
    try {
      ...
      if (initResponse.status === 200 && initResponse.body.success) {
        ...
        if (hmrResponse.status === 200 && hmrResponse.body.success) {
          ...
          console.log(`   ❌ Internal API HMR failed: ${hmrResponse.body?.error || 'Unknown error'}`);
        }
      } else {
        console.log(`   ⚠️  Internal API initialization failed: ${initResponse.body?.error || 'Unknown error'}`);
        console.log('      This is expected if setupDevBundler approach is not accessible');
      }
    } catch (error) {
  ...
  console.log(`File-based HMR API:    ${fileBasedWorking ? '✅ WORKING' : '❌ NOT AVAILABLE'}`);
  console.log(`Internal API HMR:      ${internalAPIWorking ? '✅ WORKING' : '❌ NOT AVAILABLE'}`);
