...
test("HMR Methods Comparison Test", async (t) => {
  ...
  try {
    ...
    const fileAPITest = await makeRequest("/api/file-based-hmr", "POST", {
      action: "test",
    });
    ...
    if (fileAPITest.status === 200) {
      ...
      if (fileHMRTrigger.status === 200 && fileHMRTrigger.body.success) {
        ...
        if (updatedPage1.body.includes("Hello world!!")) {
          ...
          console.log(
            "    ✅ Method 1: Page successfully updated via file-based HMR",
          );
        } else {
          console.log(
            "    ❌ Method 1: Page not updated despite successful trigger",
          );
        }
      } else {
        results.method1.error = fileHMRTrigger.body?.error || "Trigger failed";
        console.log(
          "    ❌ File-based HMR trigger failed:",
          results.method1.error,
        );
      }
    } else {
  ...
  try {
    ...
    if (internalAPITest.status === 200) {
      ...
      if (initResponse.status === 200 && initResponse.body.success) {
        ...
        const internalHMRTrigger = await makeRequest(
        ...
        if (
          internalHMRTrigger.status === 200 &&
          internalHMRTrigger.body.success
        ) {
          ...
          if (updatedPage2.body.includes("Hello world!!")) {
            ...
            console.log(
              "    ✅ Method 2: Page successfully updated via internal API HMR",
            );
          } else {
            console.log(
              "    ❌ Method 2: Page not updated despite successful trigger",
            );
          }
        } else {
          results.method2.error =
            internalHMRTrigger.body?.error || "Trigger failed";
          console.log(
            "    ❌ Internal API HMR trigger failed:",
            results.method2.error,
          );
        }
      } else {
        results.method2.error =
          initResponse.body?.error || "Initialization failed";
        console.log(
          "    ❌ Internal API initialization failed:",
          results.method2.error,
        );
      }
    } else {
  ...
  console.log("==================================");
  console.log(
    `Method 1 (File-Based):    Available: ${results.method1.available ? "✅" : "❌"}  Successful: ${results.method1.successful ? "✅" : "❌"}`,
  );
  console.log(
    `Method 2 (Internal API):  Available: ${results.method2.available ? "✅" : "❌"}  Successful: ${results.method2.successful ? "✅" : "❌"}`,
  );

  if (results.method1.error)
    console.log(`Method 1 Error: ${results.method1.error}`);
  if (results.method2.error)
    console.log(`Method 2 Error: ${results.method2.error}`);
