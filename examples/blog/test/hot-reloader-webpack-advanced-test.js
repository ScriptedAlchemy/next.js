...
function findNextServerInstances() {
  try {
    ...
    const nextGlobals = ['__NEXT_DATA__', '__next', '__NEXT_ROUTER_READY', '__NEXT_GLOBAL__'];
    nextGlobals.forEach(globalName => {
      if (global[globalName]) {
        console.log(`   ✅ Found global.${globalName}:`, typeof global[globalName]);
      }
    });
    
    // Check module cache for any dev server related modules
    const cache = require.cache;
    const devServerModules = Object.keys(cache).filter(key => 
      key.includes('dev-server') || 
      key.includes('hot-reloader') ||
      key.includes('next/server')
    );
    
    console.log(`   Found ${devServerModules.length} dev-server related modules in cache`);
...
try {
  ...
  console.log('   ✅ Found webpack bundler function:', typeof webpackBundler.default);
  
  // Try to access webpack config utilities
  const getWebpackConfig = require('next/dist/build/webpack-config');
  console.log('   ✅ Found webpack config function:', typeof getWebpackConfig.default);
  
  // Try to access hot middleware
  const webpackHotMiddleware = require('next/dist/server/dev/hot-middleware');
  console.log('   ✅ Found hot middleware:', typeof webpackHotMiddleware.WebpackHotMiddleware);
  ...
  try {
    ...
    for (const pageConfig of testPages) {
      try {
        ...
        console.log(`   ❌ ensurePage("${pageConfig.page}") failed:`, error.message);
      }
    }
...
async function runAllTests() {
  ...
  console.log('✅ Basic HMR actions can be created (but not sent without middleware)');
  ...
  console.log('\nKEY FINDINGS:');
  console.log('- Class requires complete Next.js dev environment to be fully functional');
  ...
  console.log('- ensurePage() works but requires onDemandEntries setup for full functionality');
