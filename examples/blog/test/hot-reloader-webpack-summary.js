  ...
  try {
    ...
    testActions.forEach((action, index) => {
      try {
        ...
        console.log(
          `   ❌ HMR Action ${index + 1}: ${action.action} failed - ${error.message}`,
        );
        testResults.hmrActions.push({
...
relatedModules.forEach((moduleName) => {
  try {
    ...
    if (module.WebpackHotMiddleware) {
      console.log(
        `      - WebpackHotMiddleware: ${typeof module.WebpackHotMiddleware}`,
      );
    }
    if (module.HMR_ACTIONS_SENT_TO_BROWSER) {
      console.log(
        `      - HMR_ACTIONS: ${Object.keys(module.HMR_ACTIONS_SENT_TO_BROWSER).length} actions`,
      );
    }
  } catch (error) {
...
setTimeout(() => {
  ...
  console.log("\n📋 WHAT WORKS:");
  console.log(
    "✅ Import HotReloaderWebpack from next/dist/server/dev/hot-reloader-webpack",
  );
  console.log("✅ Create instances with proper constructor parameters");
  console.log(
    "✅ Call basic methods: setHmrServerError, clearHmrServerError, invalidate, close",
  );
  ...
  console.log(
    `Instantiation: ${testResults.instantiation ? "SUCCESS" : "FAILED"}`,
  );
  console.log(
    `Basic Methods: ${testResults.basicMethods.filter((m) => m.success).length}/${testResults.basicMethods.length} working`,
  );
  console.log(
    `Advanced Methods: ${testResults.advancedMethods.filter((m) => m.success).length}/${testResults.advancedMethods.length} working`,
  );
  console.log(
    `HMR Actions: ${testResults.hmrActions.filter((a) => a.success).length}/${testResults.hmrActions.length} working`,
  );
  console.log(
    `Global Access: ${testResults.globalAccess ? "FOUND" : "NOT FOUND"}`,
  );
