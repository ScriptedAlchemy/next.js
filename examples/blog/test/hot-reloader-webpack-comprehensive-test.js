...
try {
  ...
  const prototypeMethods = Object.getOwnPropertyNames(HotReloaderWebpack.prototype)
...
try {
  ...
  console.log('\n📊 Public properties:');
  const publicProps = ['serverStats', 'edgeServerStats', 'multiCompiler', 'activeWebpackConfigs'];
  publicProps.forEach(prop => {
    const value = testInstance[prop];
    console.log(`   ${prop}:`, value === null ? 'null' : value === undefined ? 'undefined' : typeof value);
  });
...
try {
  ...
  if (typeof global !== 'undefined') {
    Object.keys(global).forEach(key => {
      if (key.toLowerCase().includes('hot') || 
  ...
  console.log('   Global keys related to hot reloading:', globalHotReloaderKeys);
  
  // Check process object
  const processKeys = [];
  if (typeof process !== 'undefined') {
    Object.keys(process).forEach(key => {
      if (key.includes('next') || key.includes('hot') || key.includes('webpack')) {
  ...
  const hotReloaderModules = Object.keys(moduleCache).filter(key => 
  ...
  testResults.globalRefs = globalHotReloaderKeys.length > 0 || processKeys.length > 0;
} catch (error) {
    ...
    try {
      ...
      while (obj && obj !== Object.prototype) {
        Object.getOwnPropertyNames(obj).forEach(name => {
          if (typeof testInstance[name] === 'function' && name !== 'constructor' && !allMethods.includes(name)) {
      ...
      const safeMethodTests = [
        ...
        },
        {
          ...
          test: () => testInstance.invalidate({ reloadAfterInvalidation: true }),
      ...
      for (const methodTest of safeMethodTests) {
        try {
          methodTest.test();
          console.log(`   ✅ ${methodTest.name}: ${methodTest.description} - SUCCESS`);
          successfulMethods++;
        } catch (error) {
          console.log(`   ❌ ${methodTest.name}: ${methodTest.description} - FAILED: ${error.message}`);
        }
      }
      ...
      try {
        ...
        console.log(`   ✅ getCompilationErrors: Get compilation errors for test page - SUCCESS (Returned ${Array.isArray(errors) ? errors.length : 'non-array'} errors)`);
        successfulMethods++;
      } catch (error) {
        console.log(`   ❌ getCompilationErrors: Get compilation errors for test page - FAILED: ${error.message}`);
      }
      ...
      try {
        ...
        console.log(`   ⚠️  send: Send HMR action - EXPECTED FAILURE: ${error.message}`);
        console.log(`      (This method requires full webpack/middleware setup)`);
      }
      
      // Test ensurePage method
      try {
        await testInstance.ensurePage({ page: '/', clientOnly: true });
        console.log(`   ✅ ensurePage: Ensure page compilation - SUCCESS (Page ensured)`);
        successfulMethods++;
      } catch (error) {
        console.log(`   ⚠️  ensurePage: Ensure page compilation - EXPECTED FAILURE: ${error.message}`);
        console.log(`      (This method requires full webpack/middleware setup)`);
      }
  ...
  try {
    ...
    try {
      ...
      if (hmrTypes.HMR_ACTIONS_SENT_TO_BROWSER) {
        ...
        Object.entries(hmrTypes.HMR_ACTIONS_SENT_TO_BROWSER).forEach(([key, value]) => {
...
console.log(`\n📋 WHAT WORKS:`);
console.log(`   ✅ Can import HotReloaderWebpack from 'next/dist/server/dev/hot-reloader-webpack'`);
...
console.log(`   ✅ Can call basic methods like setHmrServerError, clearHmrServerError, invalidate`);
console.log(`   ✅ Can access additional utility functions like getVersionInfo`);
...
console.log(`   ⛔ Class must be manually instantiated with complete configuration`);
console.log(`   ⛔ Advanced methods fail without full Next.js dev server environment`);
...
console.log(`\n💡 RECOMMENDATIONS:`);
console.log(`   1. Use HotReloaderWebpack within a proper Next.js dev server context`);
console.log(`   2. Ensure all required dependencies are properly configured`);
console.log(`   3. Mock dependencies carefully for testing isolated functionality`);
console.log(`   4. Consider using Next.js CLI commands instead of direct class access`);
console.log(`   5. For HMR functionality, work with the full Next.js development server`);
