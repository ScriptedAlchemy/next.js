...
test("Minimal onDemandEntryHandler exploration", async (t) => {
  ...
  try {
    ...
    console.log(
      "✓ onDemandEntryHandler type:",
      typeof OnDemandModule.onDemandEntryHandler,
    );

    // Step 2: Examine the entry management functions
    console.log("\n2. Testing entry management functions...");
    const { getEntries, getInvalidator, getEntryKey, ADDED, BUILDING, BUILT } =
      OnDemandModule;
    ...
    console.log(
      "✓ getInvalidator() returns:",
      invalidator ? "Invalidator instance" : "null (expected for new dir)",
    );
    ...
    const mockMultiCompiler = {
      ...
      compilers: [
        {
          ...
          hooks: {
            make: {
              tap: (name, fn) =>
                console.log(`Hooked into ${name} for client compiler`),
            },
            done: {
              tap: (name, fn) =>
                console.log(`Hooked into ${name} for client compiler done`),
            },
          },
        },
        {
          ...
          hooks: {
            make: {
              tap: (name, fn) =>
                console.log(`Hooked into ${name} for server compiler`),
            },
            done: {
              tap: (name, fn) =>
                console.log(`Hooked into ${name} for server compiler done`),
            },
          },
        },
      ],
      hooks: {
          ...
          tap: (name, fn) =>
            console.log(`Hooked into ${name} for multi-compiler done`),
        },
      },
    };
    ...
    const mockHotReloader = {
      send: (data) =>
        console.log("📡 HotReloader.send:", JSON.stringify(data, null, 2)),
    };
    ...
    console.log(
      "✓ Created mocks for MultiCompiler, HotReloader, and NextConfig",
    );
    ...
    console.log(
      "✓ Current entries after operations:",
    ...
    console.log("\n=== RESULTS & INSIGHTS ===");
    console.log(
      "✅ SUCCESS: onDemandEntryHandler can be imported and initialized",
    );
    ...
    console.log(
      "  ✓ ensurePage(options): Promise<void> - triggers page compilation",
    );
    console.log(
      "  ✓ onHMR(client, getHmrServerError): void - sets up HMR handling",
    );
    console.log("");
    console.log(
      "🎯 CONCLUSION: onDemandEntryHandler provides a working path to",
    );
  ...
  try {
    ...
    const mockMultiCompiler = {
      ...
      compilers: [
        {
          ...
          hooks: {
            make: {
              tap: (name, fn) => {
                console.log(
                  `🔧 Webpack hook registered: ${name} on client compiler`,
                );
              ...
              tap: (name, fn) => {
                console.log(
                  `🔧 Webpack hook registered: ${name} on client compiler done`,
                );
      ...
      hooks: {
          ...
          tap: (name, fn) => {
            console.log(
              `🔧 Webpack hook registered: ${name} on multi-compiler`,
            );
    ...
    try {
      ...
      onDemandEntries
        ...
        .catch((err) => {
          console.log(
            `   ⚠ Expected error (no real files): ${err.message.split("\n")[0]}`,
          );
        });
    } catch (err) {
