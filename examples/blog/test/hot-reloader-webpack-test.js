...
try {
  console.log('   Function toString length:', HotReloaderWebpack.toString().length);
  console.log('   Prototype methods:', Object.getOwnPropertyNames(HotReloaderWebpack.prototype));
...
try {
  ...
  publicProps.forEach(prop => {
    console.log(`   - ${prop}:`, typeof instance[prop], instance[prop] !== undefined ? '(defined)' : '(undefined)');
  });
...
(async () => {
  try {
    ...
    try {
      ...
      console.log('   ✅ getCompilationErrors(): Success, returned:', Array.isArray(errors) ? `Array(${errors.length})` : typeof errors);
    } catch (error) {
    ...
    try {
      ...
      if (fallbackPromise && typeof fallbackPromise.then === 'function') {
        ...
        if (result === 'timeout') {
          console.log('   ⚠️  buildFallbackError(): Timed out (expected - requires full setup)');
        } else {
...
try {
  ...
  const hotReloaderModules = Object.keys(moduleCache).filter(key => 
    key.includes('hot-reloader') || key.includes('webpack')
  );
  console.log('   Hot reloader related modules in cache:', hotReloaderModules.length);
  
  // Try to access version info function
  try {
    const { getVersionInfo } = require('next/dist/server/dev/hot-reloader-webpack');
    if (getVersionInfo) {
...
console.log('✅ Can call basic methods like setHmrServerError, clearHmrServerError, invalidate, close');
console.log('⚠️  Advanced methods require full Next.js dev server setup');
console.log('⚠️  Some methods may hang or fail without webpack compiler instances');
console.log('⚠️  No global instances found - class must be instantiated manually');
console.log('\nRECOMMENDATION:');
