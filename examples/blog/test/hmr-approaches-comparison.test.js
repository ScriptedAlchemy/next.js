...
test("HMR Approaches Comparison Test", async (t) => {
  ...
  const results = {
    fileBased: {
      available: false,
      performance: null,
      approach: "pre-built-chunks",
    },
    internalAPI: {
      available: false,
      performance: null,
      approach: "new-compiler",
    },
    efficient: {
      available: false,
      performance: null,
      approach: "existing-instance",
    },
  };
  ...
  try {
    ...
    if (internalAPITest.status === 200) {
      ...
      if (initResponse.status === 200 && initResponse.body.success) {
        ...
        console.log(
          "⚠️  Internal API HMR: Available but initialization failed",
        );
  ...
  try {
    ...
    if (efficientAPITest.status === 200) {
      ...
      if (initResponse.status === 200 && initResponse.body.success) {
        ...
        console.log(
          `   Memory Efficient: ${initResponse.body.performance?.memoryEfficient}`,
        );
        console.log(
          `   Reuses Compiler: ${initResponse.body.performance?.reusesExistingInstance}`,
        );
      } else {
  ...
  approaches.forEach(({ name, data }) => {
    ...
    if (data.available && data.performance) {
      ...
      console.log(
        `   Compilation Overhead: ${data.performance.compilationOverhead}`,
      );
  ...
  console.log("============================");
  console.log(
    "File-Based HMR:    ✅ External testing, CI/CD, simple automation",
  );
  console.log(
    "Internal API HMR:  ⚠️  Learning/research, when other methods fail",
  );
  ...
  assert.ok(
    availableCount > 0,
    "At least one HMR approach should be available",
  );

  if (results.efficient.available && results.internalAPI.available) {
    console.log(
      "✨ EXCELLENT: Both efficient and internal API approaches working!",
    );
