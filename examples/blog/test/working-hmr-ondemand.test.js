...
`;
  ...
  try {
    ...
    const { onDemandEntryHandler, getEntries, findPagePathData } = require('next/dist/server/dev/on-demand-entry-handler');
    ...
    const mockHotReloader = {
      send: (data) => {
        ...
        console.log('📡 HMR Message:', data.action, data.errorJSON ? '(with error)' : '');
      }
    };
    ...
    const hasUpdatedContent = currentContent.includes('Updated HMR Test Page via onDemandEntryHandler!');
...
test('onDemandEntryHandler integration patterns', async (t) => {
  ...
  try {
    ...
    const { onDemandEntryHandler, getEntries, getInvalidator } = require('next/dist/server/dev/on-demand-entry-handler');
    
    console.log('1. 📋 PATTERN: Standalone Compilation Service');
    console.log('   Use Case: Background compilation without dev server');
    console.log('   Setup: onDemandEntryHandler + minimal webpack + file watcher');
    ...
    console.log('   Setup: onDemandEntryHandler + custom WebSocket + file monitoring');
    ...
    console.log('   Setup: onDemandEntryHandler + build tool hooks + entry management');
    ...
    console.log('   Setup: onDemandEntryHandler + dev server + custom middleware');
    ...
    console.log('const { onDemandEntryHandler } = require("next/dist/server/dev/on-demand-entry-handler");');
    console.log('const webpack = require("next/dist/compiled/webpack/webpack");');
    console.log('');
    ...
    console.log('  hotReloader: { send: (data) => { /* handle HMR messages */ } },');
    ...
    console.log('await onDemandEntries.ensurePage({ page: "/my-page", isApp: false });');
    console.log('');
    ...
    console.log('   2. Create minimal webpack setup that works with onDemandEntryHandler');
    ...
    console.log('   that can enable programmatic HMR with proper webpack setup!');
