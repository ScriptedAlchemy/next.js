...
async function testWorkingSetupDevBundler() {
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(
        () => reject(new Error("Setup timeout after 60 seconds")),
        60000,
      );
    });
    ...
    if (devBundler.hotReloader) {
      ...
      const uniqueMethods = [...new Set(hotReloaderMethods)].filter(
        (name) =>
...
testWorkingSetupDevBundler()
  .then((result) => {
    if (result) {
      ...
      console.log(
        "🎯 You can now use setupDevBundler to get programmatic HMR access!",
      );
