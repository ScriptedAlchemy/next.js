...
async function waitForServer(port, maxRetries = 30) {
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      console.log(`Waiting for server on port ${port}... (${i + 1}/${maxRetries})`);
      await delay(2000);
    }
  }
  return false;
...
async function runHMRTests() {
  ...
  try {
    ...
    if (updatedResponse.status === 200) {
      const hasHelloWorld = updatedResponse.body.includes('<h1>Hello World</h1>');
      console.log(`   Still contains "Hello World": ${hasHelloWorld ? '✅' : '❌'}`);
    }

    // Test 4: Test server HMR status
    console.log('6. Testing server HMR status...');
    try {
      const serverHMRResponse = await testAPI('/api/server-hmr', 'POST', { action: 'test' });
      console.log(`   Server HMR Status: ${serverHMRResponse.status}`);
      if (serverHMRResponse.body) {
        console.log(`   Available functions:`, serverHMRResponse.body.availableFunctions || 'N/A');
      }
    } catch (error) {
    ...
    try {
      ...
      if (simpleHMRResponse.body) {
        console.log(`   Capabilities:`, simpleHMRResponse.body.capabilities || 'N/A');
      }
    } catch (error) {
