...
async function testHMR() {
  ...
  try {
    ...
    if (initialPage.status === 200) {
      ...
      console.log(
        `   Contains "Hello World": ${hasHelloWorld ? "✅ YES" : "❌ NO"}`,
      );
    }
    ...
    if (serverHMR.body && serverHMR.body.availableFunctions) {
      console.log(
        `   ✅ Available functions: ${serverHMR.body.availableFunctions.join(", ")}`,
      );
    }
    ...
    if (hmrTrigger.body) {
      ...
      console.log(
        `   Success: ${hmrTrigger.body.success ? "✅ YES" : "❌ NO"}`,
      );
    ...
    try {
      ...
      console.log(
        `   Simple HMR: ${simpleHMR.status} ${simpleHMR.status === 200 ? "✅" : "❌"}`,
      );
    } catch (error) {
    ...
    try {
      ...
      console.log(
        `   Chunk Factory: ${chunkFactory.status} ${chunkFactory.status === 200 ? "✅" : "❌"}`,
      );
    } catch (error) {
    ...
    try {
      ...
      console.log(
        `   Enhanced HMR: ${enhancedHMR.status} ${enhancedHMR.status === 200 ? "✅" : "❌"}`,
      );
    } catch (error) {
    ...
    console.log("=====================");
    console.log(
      `The HMR system is ${hmrTrigger.body && hmrTrigger.body.success ? "WORKING" : "NOT WORKING"} on port ${serverPort}`,
    );
  } catch (error) {
    console.error("\n❌ Test failed:", error.message);
    console.log(
      "\n💡 Make sure the Next.js dev server is running on port",
      serverPort,
    );
