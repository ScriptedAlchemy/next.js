...
export default async function handler(req, res) {
  ...
  if (process.env.NODE_ENV !== 'development') {
    return res.status(400).json({ error: 'File-based HMR API only available in development' });
  }
...
async function triggerFileBasedHMR(pagePath, res) {
  try {
    ...
    try {
      ...
      console.log(`[File-based HMR] Read compiled document: ${originalContent.length} bytes`);
    } catch (error) {
    ...
    try {
      if (global.__SERVER_HMR__ && global.__SERVER_HMR__.clearModuleCache) {
        ...
        console.log('[File-based HMR] Triggered cache invalidation via Server HMR API');
      } else {
    ...
    console.error(`[File-based HMR] Error triggering HMR for ${pagePath}:`, error);
    return res.status(500).json({
      success: false,
