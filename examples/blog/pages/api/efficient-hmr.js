...
export default async function handler(req, res) {
  ...
  if (process.env.NODE_ENV !== 'development') {
    return res.status(400).json({ error: 'Efficient HMR only available in development' });
  }

  const { action, pagePath, forceReload, customAction, customData, modules } = req.body;
    ...
    switch (action) {
      case 'test':
        return res.json({
          ...
          availableActions: ['test', 'initialize', 'trigger-hmr', 'status', 'send-message', 'invalidate'],
  ...
  try {
    ...
    if (!efficientHMRInstance.isAvailable()) {
      return res.status(503).json({
        ...
        suggestion: 'Initialization may have failed or hot reloader not accessible'
      });
    }

    console.log(`[Efficient HMR API] Triggering efficient HMR for: ${pagePath}`);
    ...
    console.error(`[Efficient HMR API] Error triggering HMR for ${pagePath}:`, error);
    return res.status(500).json({
      success: false,
