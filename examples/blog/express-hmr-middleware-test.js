...
function createHMRMiddleware(options = {}) {
  ...
  middleware.init = (compilers, versionInfo, devtoolsUrl) => {
    ...
    hotMiddleware = new WebpackHotMiddleware(compilers, versionInfo, devtoolsUrl);
    isInitialized = true;
  ...
  middleware.handleUpgrade = (request, socket, head) => {
    ...
    const mockClient = {
      ...
      addEventListener: (event, callback) => console.log(`📝 WebSocket ${event} listener added`)
    };
...
function testGlobalHMRRegistry() {
  ...
  if (typeof global !== 'undefined') {
    ...
    console.log('✅ Global HMR registry attached to global.__NEXT_HMR_REGISTRY__');
  }
...
async function runTests() {
  ...
  hmrMiddleware.init(mockCompilers, { version: '14.0.0' }, 'http://localhost:3000');
  ...
  mockRequests.forEach((req, index) => {
    ...
    const res = { setHeader: (key, value) => console.log(`     Header set: ${key}=${value}`) };
  ...
  registry.broadcast(HMR_ACTIONS_SENT_TO_BROWSER.RELOAD_PAGE, { data: '/test-page' });
  ...
  setTimeout(() => {
    ...
    if (global.__NEXT_HMR_REGISTRY__) {
      ...
      console.log('   Available middlewares:', global.__NEXT_HMR_REGISTRY__.getAllMiddlewares());
