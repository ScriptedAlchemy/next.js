...
class DevServerAccessor extends EventEmitter {
    ...
    async getServerFromRenderServerRegistry(dir = this.options.projectDir) {
        try {
            ...
            delete require.cache[require.resolve('./node_modules/next/dist/server/lib/render-server.js')];
            ...
            if (typeof renderServer.getServerField === 'function') {
                try {
                    const serverInstance = await renderServer.getServerField(dir, 'server');
                    ...
                    if (serverInstance) {
                        ...
                        this.serverInstances.set(dir, {
                            ...
                            accessedAt: new Date(),
                            methods: Object.getOwnPropertyNames(serverInstance).filter(name => 
                                typeof serverInstance[name] === 'function'
                            )
                        });
                    ...
                    this.log('❌ Server not initialized in render-server registry:', serverError.message);
                }
            }
    ...
    setupServerCaptureHooks() {
        try {
            ...
            const nextServerPath = require.resolve('./node_modules/next/dist/server/next.js');
            const renderServerPath = require.resolve('./node_modules/next/dist/server/lib/render-server.js');
    ...
    startProcessMonitoring() {
        ...
        require('child_process').spawn = (...args) => {
            ...
            if (command.includes('node') && cmdArgs && 
    ...
    async pollForServers() {
        ...
        if (process.title.includes('next-server') || process.title.includes('next-render-worker')) {
...
async function demo() {
    ...
    console.log('💡 Start your Next.js dev server with "npm run dev" to see it in action!');
    ...
    setInterval(() => {
        ...
        if (instances.length > 0) {
            console.log(`📊 Currently tracking ${instances.length} server instance(s):`);
