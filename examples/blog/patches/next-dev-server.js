...
const _nextserver = /*#__PURE__*/ _interop_require_wildcard(
  require("../next-server"),
);
...
const _log = /*#__PURE__*/ _interop_require_wildcard(
  require("../../build/output/log"),
);
const _iserror = /*#__PURE__*/ _interop_require_wildcard(
  require("../../lib/is-error"),
);
...
const ReactDevOverlay = (props) => {
  if (ReactDevOverlayImpl === undefined) {
    ReactDevOverlayImpl =
      require("../../client/components/react-dev-overlay/pages/pages-dev-overlay").PagesDevOverlay;
  }
  return ReactDevOverlayImpl(props);
};
class DevServer extends _nextserver.default {
  getStaticPathsWorker() {
    const worker = new _jestworker.Worker(
  ...
  constructor(options) {
    ...
    this.startServerSpan =
      options.startServerSpan ?? (0, _trace.trace)("start-next-dev-server");
    ...
    this.staticPathsCache = new _lrucache.LRUCache( // 5MB
      ...
      function length(value) {
        var _JSON_stringify;
        return (
          ((_JSON_stringify = JSON.stringify(value.staticPaths)) == null
            ? void 0
            : _JSON_stringify.length) ?? 0
        );
      },
    );
    this.renderOpts.ampSkipValidation =
      ((_this_nextConfig_experimental = this.nextConfig.experimental) == null
        ? void 0
        : (_this_nextConfig_experimental_amp =
              _this_nextConfig_experimental.amp) == null
          ? void 0
          : _this_nextConfig_experimental_amp.skipValidation) ?? false;
    ...
    if (this.nextConfig.experimental.serverComponentsHmrCache) {
      this.serverComponentsHmrCache = new _lrucache.LRUCache(
  ...
  getRouteMatchers() {
    ...
    const matchers = new _devroutematchermanager.DevRouteMatcherManager(
    ...
    if (pagesDir) {
      const fileReader = new _batchedfilereader.BatchedFileReader(
        new _defaultfilereader.DefaultFileReader({
          // Only allow files that have the correct extensions.
          pathnameFilter: (pathname) => extensionsExpression.test(pathname),
        }),
      );
      matchers.push(
        new _devpagesroutematcherprovider.DevPagesRouteMatcherProvider(
          pagesDir,
          extensions,
          fileReader,
      ...
      matchers.push(
        new _devpagesapiroutematcherprovider.DevPagesAPIRouteMatcherProvider(
          pagesDir,
          extensions,
          fileReader,
          this.localeNormalizer,
    ...
    if (appDir) {
      ...
      matchers.push(
        new _devapppageroutematcherprovider.DevAppPageRouteMatcherProvider(
          appDir,
          extensions,
          fileReader,
        ),
      ...
      matchers.push(
        new _devapprouteroutematcherprovider.DevAppRouteRouteMatcherProvider(
          appDir,
          extensions,
          fileReader,
        ),
  ...
  async prepareImpl() {
    ...
    // Expose hot reloader for efficient HMR access
    if (
      this.bundlerService &&
      this.bundlerService.bundler &&
      this.bundlerService.bundler.hotReloader
    ) {
      global.__NEXT_DEV_HOT_RELOADER__ =
        this.bundlerService.bundler.hotReloader;
  ...
  async hasPage(pathname) {
    ...
    if ((0, _utils2.isMiddlewareFile)(normalizedPath)) {
      return (0, _findpagefile.findPageFile)(
        this.dir,
        normalizedPath,
    ...
    let pagesFile = null;
    if (this.appDir) {
      appFile = await (0, _findpagefile.findPageFile)(
        this.appDir,
        normalizedPath + "/page",
        this.nextConfig.pageExtensions,
        true,
    ...
    if (this.pagesDir) {
      pagesFile = await (0, _findpagefile.findPageFile)(
        this.pagesDir,
        normalizedPath,
        this.nextConfig.pageExtensions,
        false,
  ...
  async runMiddleware(params) {
    try {
      ...
      (0, _errorsource.decorateServerError)(
        err,
  ...
  getRequestHandler() {
    ...
    return (req, res, parsedUrl) => {
      ...
      if (loggingConfig !== false) {
        ...
        const isMiddlewareRequest = (0, _requestmeta.getRequestMeta)(
          req,
          "middlewareInvoke",
        );
    ...
    if (pathname.startsWith("/_next")) {
      if (_fs.default.existsSync((0, _path.join)(this.publicDir, "_next"))) {
        throw Object.defineProperty(
          new Error(_constants.PUBLIC_DIR_MIDDLEWARE_CONFLICT),
          "__NEXT_ERROR_CODE",
          {
            value: "E394",
            enumerable: false,
  ...
  getPagesManifest() {
    return (
      _nodemanifestloader.NodeManifestLoader.require(
        (0, _path.join)(this.serverDistDir, _constants1.PAGES_MANIFEST),
      ) ?? undefined
    );
  }
  getAppPathsManifest() {
    ...
    return (
      _nodemanifestloader.NodeManifestLoader.require(
        (0, _path.join)(this.serverDistDir, _constants1.APP_PATHS_MANIFEST),
      ) ?? undefined
    );
  }
  getinterceptionRoutePatterns() {
    const rewrites = (0,
    _generateinterceptionroutesrewrites.generateInterceptionRoutesRewrites)(
      Object.keys(this.appPathRoutes ?? {}),
      this.nextConfig.basePath,
    ).map(
      (route) =>
        new RegExp(
          (0, _buildcustomroute.buildCustomRoute)("rewrite", route).regex,
        ),
    );
  ...
  async getMiddleware() {
    ...
    if (
      ((_this_middleware = this.middleware) == null
        ? void 0
        : _this_middleware.match) === null
    ) {
      this.middleware.match = (0,
      _middlewareroutematcher.getMiddlewareRouteMatcher)(
      ...
      try {
        instrumentationModule = await require(
          (0, _path.join)(
            this.distDir,
            "server",
            _constants.INSTRUMENTATION_HOOK_FILENAME,
          ),
        );
      } catch (err) {
  ...
  async runInstrumentationHookIfAvailable() {
    await this.startServerSpan
      ...
      .traceAsyncFn(() => {
        ...
        return (_this_instrumentation = this.instrumentation) == null
          ? void 0
          : (_this_instrumentation_register = _this_instrumentation.register) ==
              null
            ? void 0
            : _this_instrumentation_register.call(_this_instrumentation);
      });
  }
  ...
  async getStaticPaths({ pathname, requestHeaders, page, isAppPath }) {
    ...
    const __getStaticPaths = async () => {
      ...
      try {
        ...
        const pathsResult = await staticPathsWorker.loadStaticPaths({
          ...
          sriEnabled: Boolean(
            (_this_nextConfig_experimental_sri =
              this.nextConfig.experimental.sri) == null
              ? void 0
              : _this_nextConfig_experimental_sri.algorithm,
          ),
        });
        return pathsResult;
      } finally {
    ...
    const nextInvoke = (0, _coalescedfunction.withCoalescedInvoke)(
      __getStaticPaths,
      ...
      .then((res) => {
        ...
        const value = {
          staticPaths:
            staticPaths == null
              ? void 0
              : staticPaths.map((route) => route.pathname),
  ...
  async getFallbackErrorComponents(url) {
    ...
    return await (0, _loaddefaulterrorcomponents.loadDefaultErrorComponents)(
      this.distDir,
    );
  }
