  ...
  try {
    ...
    const createMockCompiler = (name) => ({
      ...
      watch: (options, callback) => {
        ...
        return {
          close: () => console.log(`   🔒 ${name} compiler: Stopped watching`),
        };
      },
    });
    ...
    const eventStreamSimulator = {
      ...
      addClient(clientId) {
        const client = {
          id: clientId,
          send: (data) =>
            console.log(`📤 Client ${clientId}:`, JSON.parse(data)),
        ...
        console.log(
          `👥 Client ${clientId} connected (Total: ${this.clients.size})`,
        );
        return client;
      },
...
function hookIntoProcessEvents() {
  ...
  const webpackModules = Object.keys(require.cache).filter(
    (path) => path.includes("webpack") || path.includes("hot-middleware"),
  );

  if (webpackModules.length > 0) {
    console.log(
      `   Found ${webpackModules.length} webpack-related modules in cache:`,
    );
