...
class ExistingHMRHook {
  ...
  async initialize() {
    try {
      ...
      if (global.__NEXT_DEV_SERVER_INSTANCE__) {
        ...
        this.hotReloaderInstance = this.devServerInstance.bundlerService?.bundler;
        
        if (this.hotReloaderInstance) {
          console.log('[Existing HMR Hook] Successfully hooked into existing dev server');
      ...
      for (const key in require.cache) {
        ...
        if (mod && mod.exports && typeof mod.exports === 'object') {
          if (mod.exports.constructor && mod.exports.constructor.name === 'DevServer') {
            this.devServerInstance = mod.exports;
            this.hotReloaderInstance = this.devServerInstance.bundlerService?.bundler;
            
            if (this.hotReloaderInstance) {
              console.log('[Existing HMR Hook] Found existing DevServer via require cache');
              this.initialized = true;
              return true;
            }
          }
  ...
  async triggerHMR(pagePath, forceReload = false) {
    ...
    try {
      ...
      console.error(`[Existing HMR Hook] Error triggering HMR for ${pagePath}:`, error);
      throw error;
    }
  }
  ...
  getStatus() {
    return {
      ...
      multiCompilerActive: this.hotReloaderInstance?.multiCompiler ? true : false
    };
  }
}
