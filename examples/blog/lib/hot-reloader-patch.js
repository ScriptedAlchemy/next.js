...
class HotReloaderPatch {
  ...
  applyPatches() {
    ...
    console.log(
      "[Hot Reloader Patch] Applying patches to capture existing hot reloader...",
    );
  ...
  patchDevBundlerService() {
    try {
      const DevBundlerService =
        require("next/dist/server/lib/dev-bundler-service").DevBundlerService;
      ...
      if (!DevBundlerService.prototype._originalConstructor) {
        ...
        function PatchedDevBundlerService(bundler, handler) {
          ...
          console.log(
            "[Hot Reloader Patch] Captured DevBundlerService and hot reloader",
          );
        ...
        require.cache[
          require.resolve("next/dist/server/lib/dev-bundler-service")
        ].exports.DevBundlerService = PatchedDevBundlerService;

        console.log("[Hot Reloader Patch] DevBundlerService patched");
      }
    } catch (error) {
      console.log(
        "[Hot Reloader Patch] DevBundlerService patch failed (expected):",
        error.message,
      );
    }
  }
  ...
  patchSetupDevBundler() {
    try {
      ...
      if (
        setupDevBundlerModule.setupDevBundler &&
        !setupDevBundlerModule._originalSetupDevBundler
      ) {
        setupDevBundlerModule._originalSetupDevBundler =
          setupDevBundlerModule.setupDevBundler;
        ...
        setupDevBundlerModule.setupDevBundler = async function (opts) {
          console.log(
            "[Hot Reloader Patch] setupDevBundler called, capturing result...",
          );

          const result =
            await setupDevBundlerModule._originalSetupDevBundler(opts);
          ...
          if (result && result.hotReloader) {
            ...
            console.log(
              "[Hot Reloader Patch] Hot reloader captured from setupDevBundler",
            );
          }
      ...
      console.log(
        "[Hot Reloader Patch] setupDevBundler patch failed:",
        error.message,
      );
    }
  }
  ...
  patchDevServer() {
    try {
      const NextDevServer =
        require("next/dist/server/dev/next-dev-server").default;

      if (!NextDevServer.prototype._originalPrepare) {
        NextDevServer.prototype._originalPrepare =
          NextDevServer.prototype.prepare;
        ...
        NextDevServer.prototype.prepare = async function () {
          ...
          if (
            ...
            global.__HOT_RELOADER_INSTANCE__ =
              this.bundlerService.bundler.hotReloader;
            console.log(
              "[Hot Reloader Patch] Hot reloader captured from DevServer",
            );
          }
      ...
      console.log(
        "[Hot Reloader Patch] DevServer patch failed:",
        error.message,
      );
    }
  }
  ...
  getHotReloader() {
    ...
    if (
      global.__DEV_BUNDLER_SERVICE__ &&
      global.__DEV_BUNDLER_SERVICE__.bundler
    ) {
      return global.__DEV_BUNDLER_SERVICE__.bundler.hotReloader;
    }

    if (
      global.__DEV_BUNDLER_RESULT__ &&
      global.__DEV_BUNDLER_RESULT__.hotReloader
    ) {
    ...
    if (global.__NEXT_DEV_SERVER_INSTANCE__) {
      ...
      if (
        ...
        bundlerService.bundler.hotReloader
      ) {
  ...
  getStatus() {
    ...
    return {
      ...
      hotReloaderMethods: hotReloader
        ? Object.getOwnPropertyNames(Object.getPrototypeOf(hotReloader))
        : [],
  ...
  async initialize() {
    ...
    if (hotReloader) {
      console.log(
        "[Hot Reloader Patch] Successfully captured existing hot reloader instance",
      );
      this.hotReloaderInstance = hotReloader;
      return true;
    } else {
      console.log(
        "[Hot Reloader Patch] Hot reloader not yet available, patches are waiting for Next.js startup",
      );
      return false;
    }
  }
  ...
  async triggerHMR(pagePath, forceReload = false) {
    ...
    if (!hotReloader) {
      throw new Error(
        "No hot reloader instance available - Next.js dev server may not be fully started",
      );
    }

    try {
      console.log(
        `[Hot Reloader Patch] Triggering HMR for: ${pagePath} using existing instance`,
      );
      ...
      console.error(
        `[Hot Reloader Patch] Error triggering HMR for ${pagePath}:`,
        error,
      );
      throw error;
    }
