...
function checkGlobalReferences() {
    ...
    Object.keys(process.env).filter(key => key.includes('NEXT')).forEach(key => {
...
function checkRenderServerInitializations() {
    ...
    try {
        ...
        const renderServerPath = path.join(__dirname, 'node_modules/next/dist/server/lib/render-server.js');
        ...
        if (fs.existsSync(renderServerPath)) {
            ...
            try {
                ...
                if (typeof renderServer.getServerField === 'function') {
                    renderServer.getServerField(process.cwd(), 'server')
                        .then(server => {
                            console.log('  ✅ Successfully accessed server field:', !!server);
                            if (server) {
                                console.log('  📝 Server type:', server.constructor.name);
                                console.log('  📝 Server methods:', Object.getOwnPropertyNames(server).filter(name => typeof server[name] === 'function'));
                            }
                        })
                        .catch(err => {
                ...
                console.log('  ❌ Failed to require render-server:', requireError.message);
            }
        } else {
...
function checkModuleCache() {
    ...
    const relevantModules = Object.keys(require.cache).filter(modulePath => 
...
function setupProcessEventListening() {
    ...
    process.emit = function(event, ...args) {
        if (event.toString().includes('next') || event.toString().includes('server')) {
            eventCount++;
...
function checkActiveServers() {
    ...
    commonPorts.forEach(port => {
        ...
        const req = http.request({
            ...
            console.log(`  ✅ Server found on port ${port} (status: ${res.statusCode})`);
            
            // Check for Next.js specific headers
            const nextHeaders = Object.keys(res.headers).filter(header => 
    ...
    try {
        if (fs.existsSync(nextConfigPath)) {
            fs.watchFile(nextConfigPath, { interval: 1000 }, (curr, prev) => {
                console.log('  📁 Next.js config file changed - dev server might be restarting');
            });
...
function hookNextCLI() {
    ...
    try {
        const nextDevPath = path.join(__dirname, 'node_modules/next/dist/cli/next-dev.js');
...
function tryDirectNextAPI() {
    ...
    try {
        ...
        if (typeof next === 'function') {
            ...
            if (next.prototype) {
                console.log('  📝 Next.js prototype methods:', Object.getOwnPropertyNames(next.prototype));
            }
            ...
            if (nextModule && nextModule.exports) {
                console.log('  📝 Next.js module exports:', Object.keys(nextModule.exports));
            }
        }
...
function inspectProcessRuntime() {
    ...
    if (process.env.__NEXT_PRIVATE_RENDER_WORKER) {
        console.log(`  ✅ Running in Next.js render worker: ${process.env.__NEXT_PRIVATE_RENDER_WORKER}`);
    }
    ...
    const nextProps = Object.getOwnPropertyNames(process).filter(prop => 
        prop.includes('next') || prop.includes('Next')
    );
...
async function main() {
    ...
    console.log('🏁 Test completed. Keep this script running to monitor for any server events...');
    console.log('💡 To test with a live dev server, run "npm run dev" in another terminal.\n');
