...
try {
  ...
  if (WebpackHotMiddleware) {
    ...
    console.log(
      "   Prototype methods:",
      Object.getOwnPropertyNames(WebpackHotMiddleware.prototype),
    );
    console.log(
      "   Static methods:",
      Object.getOwnPropertyNames(WebpackHotMiddleware),
    );
  } else {
...
try {
  ...
  console.log(
    "   Available HMR actions:",
    Object.keys(hmrTypes.HMR_ACTIONS_SENT_TO_BROWSER || {}),
  );

  if (hmrTypes.HMR_ACTIONS_SENT_TO_BROWSER) {
    console.log("   HMR Actions enum:");
    for (const [key, value] of Object.entries(
      hmrTypes.HMR_ACTIONS_SENT_TO_BROWSER,
    )) {
    ...
    hooks: {
      invalid: {
        tap: (pluginName, callback) => {
          console.log(
            `   ${name} compiler: registered 'invalid' hook for ${pluginName}`,
          );
        ...
        tap: (pluginName, callback) => {
          console.log(
            `   ${name} compiler: registered 'done' hook for ${pluginName}`,
          );
...
if (WebpackHotMiddleware) {
  ...
  try {
    ...
    console.log(
      "   Instance methods:",
      Object.getOwnPropertyNames(Object.getPrototypeOf(hotMiddleware)),
    );
    ...
    if (hotMiddleware.eventStream) {
      ...
      console.log(
        ...
        Object.getOwnPropertyNames(
          Object.getPrototypeOf(hotMiddleware.eventStream),
        ),
      );
      console.log(
        "   Client connections:",
        hotMiddleware.eventStream.clients?.size || 0,
      );
    }
    ...
    testEvents.forEach((event, index) => {
      try {
        ...
        console.log(
          `   ❌ Failed to publish ${event.description}:`,
          error.message,
        );
      }
    });
    ...
    try {
      ...
      console.log(
        "   ❌ Could not access Next.js server module:",
        error.message,
      );
    }

    // Check process for HMR-related globals
    const hmrGlobals = Object.keys(process).filter(
      (key) =>
    ...
    console.log(
      "❌ Failed to create WebpackHotMiddleware instance:",
      error.message,
    );
...
console.log(
  "5. Consider creating a wrapper service for centralized HMR control",
);
